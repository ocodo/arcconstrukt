...
/**
 Read a hex string (6 digits, no leading # or 0x)
...
+ (UIColor *)colorWithRGBADictionary:(NSDictionary*)rgbaDictionary {
    ...
    @try {
        r = [rgbaDictionary valueForKey:@"r"];
    ...
    @catch (NSException *exception) {
        r = [rgbaDictionary valueForKey:@"red"];
        g = [rgbaDictionary valueForKey:@"green"];
        b = [rgbaDictionary valueForKey:@"blue"];
        a = [rgbaDictionary valueForKey:@"alpha"];
...
-(NSDictionary *)RGBADictionary {
    ...
    @"g" : [NSNumber numberWithFloat:g],
    @"b" : [NSNumber numberWithFloat:b],
    @"a" : [NSNumber numberWithFloat:a]
    };
}

/**
 Return color in-between start and end colors at a given percentage point, using linear interpolation
 */
...
/**
 Return color in-between start and end colors at a given percentage point, using chromatic interpolation
 */
