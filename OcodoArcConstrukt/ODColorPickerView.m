...
-(void) defaultInitializer_ {
   ...
   self.color = [UIColor colorWithHue:1.0f saturation:1.0f brightness:0.001f alpha:1.0f];
...
-(CGPoint)indicatorCenterForSaturation:(CGFloat) saturation brightness:(CGFloat) brightness {
   ...
   CGFloat theta = M_PI_3 * saturation;

   CGFloat l = brightness * ( (sqrtf(3) / 2) / cosf(fabsf(theta - (M_PI/6))));
   ...
   CGPoint r = (CGPoint) { (v.x * cosf(theta)) - (v.y * sinf(theta)), (v.x * sinf(theta)) + (v.y * cosf(theta))};
...
-(void)onMoveHueIndicator:(UIPanGestureRecognizer *) recognizer {
   ...
   if ([recognizer state] == UIGestureRecognizerStateBegan || [recognizer state] == UIGestureRecognizerStateChanged) {
      ...
      float sat, bright, hue = (M_PI - atan2f(t.y-center.y,center.x-t.x)) / (2 * M_PI);
      [color_ getHue:NULL saturation:&sat brightness:&bright alpha:NULL];
      [self setColor:[UIColor colorWithHue:hue saturation:sat brightness:bright alpha:1.0f]];
